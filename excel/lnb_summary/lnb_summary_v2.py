...
#要处理的文件路径
fpath = "data/LNB_summary_format_V2.xlsm"
#fpath = "LNB_summary_format_V2.xlsm"#打包exe的时候改成该路径
...
def clear_d_delta(row):
    if row[('Total','Capabity.1')] == 'Delta':
        print(f"清除{row[('Total','Capabity')]}的{row[('Total','Capabity.1')]}的D列值{row[('last Q ','Rolling')]}")
        return 0
    
summary_df[('Current week','BOH')] = summary_df.apply(clear_c_Delta_Loi,axis=1)
summary_df[('last Q ','Rolling')] = summary_df.apply(clear_d_delta,axis=1)
...
def Cal_D_Delta_Iter_In_Row(sum_row,sd_df,ob_df):
    ...
    if sum_b_value == 'Delta':
        ...
        selected_sd_rows = sd_df.loc[(sd_df[('Total','Capabity')] == sum_item_group) & (sd_df[('Total','Capabity.1')] == 'Delta')]
        for index_row,selected_sd_row in selected_sd_rows.iterrows():
            sd_delta = selected_sd_row[('last Q ','Rolling')]
...
summary_df[('last Q ','Rolling')] = summary_df.apply(Cal_D_Delta_Iter_In_Row,axis=1,args=(sd_df,ob_df))
