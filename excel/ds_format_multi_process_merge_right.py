...
def p_clear_and_cal_delta(q):
    ...
    print(f"清空DS表的Delta列的值 time cost is :{clear_delta_end - clear_delta_start} seconds")
    #计算开始
    ...
    print(f"计算DS表的Delta列的值 time cost is :{cal_delta_end - cal_delta_start} seconds")

def p_clear_and_cal_loi(q):
    #读取excel数据到内存
    read_excel()
    #先处理一下item_group的nan值
    handle_nan_item_group()
    ...
    print(f"清空DS表的LOI列的值 time cost is :{clear_delta_loi_end - clear_delta_loi_start} seconds")
    #计算开始
    ...
    print(f"计算DS表的LOI列的值 time cost is :{cal_delta_loi_end - cal_delta_loi_start} seconds")
...
def p_clear_and_cal_demand(q):
    #读取excel数据到内存
    read_excel()
    #先处理一下item_group的nan值
    handle_nan_item_group()
    clear_demand_supply_start = time.time()
    #根据DS和CP相同的日期，清空Demand值
    ...
    print(f"清空DS表的Demand的值 time cost is :{clear_demand_supply_end - clear_demand_supply_start} seconds")  
    #计算开始
    ...
    print(f"DS表的Demand的清空和计算总共 time cost is :{cal_demand_supply_end - clear_demand_supply_start} seconds")

def p_clear_and_cal_supply(q):
    #读取excel数据到内存
    read_excel()
    #先处理一下item_group的nan值
    handle_nan_item_group()
    ...
    clear_demand_supply_end = time.time()
    print(f"清空DS表的Supply的值 time cost is :{clear_demand_supply_end - clear_demand_supply_start} seconds")  
    #计算开始
    cal_demand_supply_start = time.time()
    #根据DS和CP相同的日期，计算Demand和Supply值
    for i in range(5,len(ds_df.columns)):
    ...
    q.put(ds_df)
    #dict['ds_demand_supply'] = ds_df
    print(f"计算DS表的Supply的值 time cost is :{cal_demand_supply_end - cal_demand_supply_start} seconds")
    print(f"DS表的Supply的清空和计算总共 time cost is :{cal_demand_supply_end - clear_demand_supply_start} seconds")
...
def merge_delta_loi_demand_supply(ds_df_delta,ds_df_demand,ds_df_loi,ds_df_supply):
    ds_df_supply[('Current week','BOH')] = ds_df_supply.apply(merge_delta_loi_to_demand_supply_apply,axis=1,args=(ds_df_delta,ds_df_loi))
...
if __name__ == '__main__':
    ...
    #这里起四个进程，分别计算Delta、LOI，Demand和Supply
    p_cal_delta = Process(target=p_clear_and_cal_delta,args=(delta_q,))
    p_cal_demand = Process(target=p_clear_and_cal_demand,args=(demand_q,))
    p_cal_loi = Process(target=p_clear_and_cal_loi,args=(loi_q,))
    p_cal_supply = Process(target=p_clear_and_cal_supply,args=(supply_q,))
    ...
    ds_df_delta = delta_q.get()
    p_cal_delta.join()
   
    ds_df_demand = demand_q.get()
    p_cal_demand.join()
    ...
    merge_delta_loi_demand_supply(ds_df_delta,ds_df_demand,ds_df_loi,ds_df_supply)
    merge_end = time.time()
