...
ds_format_apply_multi_thread_io.py尝试用多线程读取excel,效果不明显
本脚本尝试用多进程读excel
"""
...
def p_read_cp_df(cp_df_queue):
    read_cp_df_start = time.time()
    global cp_df
    cp_df = pd.read_excel(fpath,sheet_name="CP",header=[0])
    cp_df_queue.put(cp_df)
    read_cp_df_end = time.time()
    print(f"Process-{os.getpid()}读取excel文件cp sheet time cost is :{read_cp_df_end - read_cp_df_start} seconds")


def p_read_ds_df(ds_df_queue):
    t_read_ds_df_start = time.time()
    global ds_df 
    ds_df = pd.read_excel(fpath,sheet_name="DS",header=[0,1])
    ds_df_queue.put(ds_df)
    t_read_ds_df_end = time.time()
    print(f"Process-{os.getpid()}读取excel文件ds sheet time cost is :{t_read_ds_df_end - t_read_ds_df_start} seconds")
...
def read_excel():
    ...
    ds_df_queue = Queue()
    read_ds_df_process = Process(target=p_read_ds_df,args=(ds_df_queue,))
    read_ds_df_process.start()

    cp_df = cp_df_queue.get()
    read_cp_df_process.join()

    ds_df = ds_df_queue.get()
    read_ds_df_process.join()
