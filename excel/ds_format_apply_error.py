...
该实现使用Pandas的函数apply()来遍历DataFrame，但是是一个对于apply()的错误使用示范
apply必须通过回调函数的返回值来修改datafram，在回调函数内部对dataframe修改是无效的
正确使用示范见ds_format_apply_right.py
"""
...
def Cal_Delta_Loi_Iter_In_Ds(ds_row):
      ...
      #获取DS表的Item_group值
        ...
        if cal_delta_loi_ds_item_group != "" and cp_item_group == cal_delta_loi_ds_item_group :
            ...
            iner_iter_df[(('Current week','BOH'))] = iner_iter_df.apply(iner_Iter_From_Cal_Delta_Loi_Iter_In_Ds,axis=1)
...
def cal_damand_Inner_Iter_Ds(inner_iter_ds_row):
    ...
    inner_iter_ds_row[('Total','Capabity')] = cal_demand_supply_cp_item_group
