...
def handle_nan_item_group(ds_df):
...
def Cal_Delta_Loi_Iter_In_Ds(ds_row,cp_df):
...
def p_clear_and_cal_delta_loi(q):
    ...
    handle_nan_item_group(ds_df)
    #清空开始
    clear_delta_loi_start = time.time()
    ds_df[('Current week','BOH')] = ds_df.apply(clear_Delta_Loi,axis=1)
    clear_delta_loi_end = time.time()
    print(f"子进程-{os.getpid()} 清空DS表的Delta和LOI列的值 time cost is :{clear_delta_loi_end - clear_delta_loi_start} seconds")
    #计算开始
    cal_delta_loi_start = time.time()
    #计算Dela和LOI的值
    ds_df[('Current week','BOH')] = ds_df.apply(Cal_Delta_Loi_Iter_In_Ds,axis=1,args=(cp_df,))
    q.put(df_dict)
    #释放数据
    delta_item_group_site_set.clear()
    loi_item_group_site_set.clear()
    cal_delta_loi_end = time.time()
    print(f"子进程-{os.getpid()} 计算DS表的Delta和LOI列的值 time cost is :{cal_delta_loi_end - cal_delta_loi_start} seconds")
...
def cal_demand_supply_by_datetime(ds_row,ds_datetime,cp_df):
    ds_item_group = ds_row[('Total','Capabity')]
    Capabity_1 = ds_row[('Total','Capabity.1')]
...
def p_clear_and_cal_demand_supply(q):
    ...
    print(f"子进程-{os.getpid()} 清空DS表的Demand和Supply的值 time cost is :{clear_demand_supply_end - clear_demand_supply_start} seconds")  
    #计算开始
    ...
    for i in range(5,len(ds_df.columns)):
        ...
        ds_month = ds_df.columns.get_level_values(0)[i]
        if type(ds_datetime) == str and ds_datetime != "" and (ds_datetime in cp_df.columns):
            ds_df[(f'{ds_month}',f'{ds_datetime}')] = ds_df.apply(cal_demand_supply_by_datetime,axis=1,args=(ds_datetime,cp_df))
    cal_demand_supply_end = time.time()
    q.put(df_dict)
    #dict['ds_demand_supply'] = ds_df
    print(f"子进程-{os.getpid()} 计算DS表的Demand和Supply的值 time cost is :{cal_demand_supply_end - cal_demand_supply_start} seconds")
    print(f"子进程-{os.getpid()} DS表的Demand和Supply的清空和计算总共 time cost is :{cal_demand_supply_end - clear_demand_supply_start} seconds")
...
def t_read_cp_df():
    ...
    print(f"主进程-{os.getpid()}-线程{threading.current_thread().getName()}读取excel文件cp sheet time cost is :{read_cp_df_end - read_cp_df_start} seconds")
...
def t_read_ds_df():
    ...
    print(f"主进程-{os.getpid()}-线程{threading.current_thread().getName()}读取excel文件ds sheet time cost is :{t_read_ds_df_end - t_read_ds_df_start} seconds")
...
if __name__ == '__main__':
    ...
    delta_loi_q = Queue()
    delta_loi_q.put(df_dict)
        
    demand_supply_q = Queue()
    demand_supply_q.put(df_dict)
    ...
    ds_df_delta_loi = delta_loi_q.get()['ds_df']
    ds_df_demand_supply = demand_supply_q.get()['ds_df']
    
    p_cal_delta_loi.join()
    p_cal_demand_supply.join()
    
    cal_end = time.time()
    print(f"主进程-{os.getpid()} ds_format python 脚本（使用多进程apply）读取数据+内存计算总共 time cost is :{cal_end - cal_start} seconds") 
    
    #将ds_delta_loi合并到ds_demand_supply
    merge_start = time.time()
    merge_delta_loi_to_demand_supply(ds_df_delta_loi,ds_df_demand_supply)
    merge_end = time.time()
    print(f"主进程-{os.getpid()} 将内存计算结果进行合并time cost is :{merge_end - merge_start} seconds")
    
    #将内存计算结果保存到excel
    save_start = time.time()
    save_excel(ds_df_demand_supply)
    save_end = time.time()
    print(f"主进程-{os.getpid()} 将内存结果保存到excel总共 time cost is :{save_end - save_start} seconds")
    print(f"主进程-{os.getpid()} ds_format python 脚本（使用多进程apply）总共 time cost is :{save_end - cal_start} seconds")
