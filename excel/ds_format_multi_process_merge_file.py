...
本方案不用多进程间通信的方式，改为多个进程分开计算，输出到文件后，主进程再读取文件进行合并的方式
"""
...
def save_excel(fpath):
    save_excel_start = time.time()
    #保存结果到excel       
    app = xw.App(visible=False,add_book=False)

    ds_format_workbook = app.books.add()
    ds_format_workbook.sheets.add('DS')
    ds_format_workbook.sheets['DS'].range("A3").expand().options(index=False).value = ds_df 

    ds_format_workbook.save(fpath)
...
def p_clear_and_cal_delta():
    ...
    #保存数据
    fpath = "datas/joyce/DS_format_bak_delta.xlsm"
    save_excel(fpath)
...
def p_clear_and_cal_loi():
    ...
    #保存数据
    fpath = "datas/joyce/DS_format_bak_loi.xlsm"
    save_excel(fpath)
...
def p_clear_and_cal_demand():
    ...
    #保存数据
    fpath = "datas/joyce/DS_format_bak_demand.xlsm"
    save_excel(fpath)
...
def p_clear_and_cal_supply():
    ...
    #保存数据
    fpath = "datas/joyce/DS_format_bak_supply.xlsm"
    save_excel(fpath)
